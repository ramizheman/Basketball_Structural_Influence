...
\usepackage{longtable} % appendix connector roster table
...
\newcommand{\load}{\ensuremath{L}} % structural load
\newcommand{\resid}{\ensuremath{r}} % usage-residual structural load
...
\title{\textbf{Structural Load: Measuring the Hidden Wiring of an NBA Offense}}
...
\begin{abstract}
\noindent
NBA teams spend billions on players, yet some of the hardest to price are the ones who
hold everyone else together. Coaches call them glue guys. Box scores barely see them.
Existing analytics measure production, efficiency, and usage, but not how an offense is
organized, or how dependent it is on any one player.

We treat that dependence as a network problem. Represent an offense as a graph linking
each player to the play types he initiates. A player's \emph{structural load} is how much
that graph's shape changes when he is removed. Crossing load against usage identifies
low-usage players whose absence forces comparable reorganization to losing a
high-usage creator: a measurable version of the glue-guy idea.

Across three NBA seasons ($924$ player-seasons, $294$ tracked player moves, methods fixed
before results were examined), this organizational importance is largely unrelated to
conventional player value. It shows up two ways. First, the menu: teams run more
cuts and pick-and-roll, and fewer spot-ups and drives, when these players are on the floor.
Second, the wiring: even holding the menu fixed, removing a connector
changes who holds which combination of jobs, an effect that partly travels with the player
after a trade. A full complement of these roles on the floor does not independently raise
scoring efficiency beyond what talent already explains, a real boundary: what an offense
depends on structurally is not the same as what makes it score. High-usage organizational
talent, by contrast, does move scoring efficiency reliably.

With this framework, researchers can ask which offenses would fall apart most if a key
player were hurt, and which trades preserve or scramble a team's identity, questions that
previously lived only as coaching intuition. Offensive organization is no longer just
something a coach senses. It can be measured.
\end{abstract}
%=============================================================================
...
number of extra points; they are decision-relevant questions about structure. Treating
...
We first introduce a reproducible graph-based framework for measuring offensive
...
responsibilities. Each player's \emph{structural load} is the change in that map when he is
removed: organizational dependence operationalized as network perturbation. Crossing load
with usage isolates \textsc{Connector/Hub} players (low usage, high residual load), the
...
player contributes to points or wins. Our objective is different: quantify how the
...
inferred indirectly from outcomes. We do not claim graphs are the only conceivable tool;
we argue they are the natural tool for this question, in the same sense that removing a
...
We then use the framework to study organizational function across three NBA seasons
...
\begin{enumerate}[leftmargin=1.6em,itemsep=0.1em]
 ...
 organizer-level structural load (Sec.~\ref{sec:emerge}).
 \item \textbf{Is it just another value metric?} Residual load is essentially orthogonal
 to net rating (Sec.~\ref{sec:orthogonality}).
 \item \textbf{Does it correspond to observable basketball?} Connector presence is
 associated with recognizable play-mix shifts (Sec.~\ref{sec:behavior}).
 \item \textbf{Where does the construct stop?} Completeness / connector presence do not
 ...
 test: connector absence displaces the observed association fingerprint more than
 ...
 \item \textbf{Can we predict when it transfers?} Destination vacancy is null under the
 measures tested; within connectors, origin cut vs.\ drive diet is a directional but
...
menus; the graph sees different organization. We model shared initiators across play types
...
\begin{figure}[H]
 ...
 \node[step] (s1) {\textbf{1. Measure:} wiring + structural load};
 ...
 (Secs.~\ref{sec:framework}--\ref{sec:travelfit}). Connectors are the visible stress-test
...
rather than entirely determined by team context. Connectors make hidden structural
...
player's contribution is his to carry elsewhere. We position organizational function
...
flow efficiency. We differ in object and question: play-type co-initiation graphs at the
...
Our data are possession-level records for three NBA regular seasons
...
possessions with a resolved initiator and a play type in our canonical set, the corpus
...
ball-handler, drive, post-up, spot-up, off-ball cut, putback, off-the-dribble/pull-up, and
...
The public play-by-play feed we parse does not tag ball-screen actions directly (no
...
our $K=8$ initiating play types and enters every association-matrix and structural-load
computation. We correct it with a calibration pass that promotes a scored subset of
...
association matrix or structural load directly. Those objects use the final calibrated
...
The \emph{initiator} is the player who begins the offensive action (ball-handler in a
pick-and-roll, cutter on a cut, shooter on a spot-up). Initiation, rather than finishing or
assisting, is the relation we graph, because it is the act that determines \emph{which player
sets each possession's structural context}.

\paragraph{Seasons, filtering, and pools.}
For role assignment we define a \emph{rotation pool} of players with at least $15$ minutes per
...
The connector-tier subgroup split in Sec.~\ref{sec:connector-tiers} was motivated by an
exploratory review of 2025--26 data (on-court coverage gap since corrected; Appendix~G lists
that season's connector roster). The check itself runs on the pre-registered three-season
sample only.
...
map; structural load measures how much the map changes when a player is removed: organizational
dependence as network perturbation. We define the offensive interaction graph, its play-type
\emph{association matrix}, and per-player \emph{structural load}. The role taxonomy is a
convenient discretization that makes connectors visible; the \emph{displacement signature} is
...
possessions initiated by player $p$. Because we care about \emph{who initiates what}, we
column-normalize so each play type is a distribution over players:
\begin{equation}
\label{eq:bnorm}
...
CUT--SPOT UP link is weak. Our measure formalizes this overlap across the full rotation.

We capture the roster map with a symmetric play-type \emph{association matrix} via the
...
avoid an implementation-time degree of freedom. We deliberately do not normalize $O$ for
volume; raw overlaps conflate genuine co-initiation with the fact that some players simply
initiate more, so we instead standardize against a \emph{fixed-margin null}: both player
...
initiations ($1{,}000$ draws). We take the mean and SD of the resulting $O$ entries. That
...
\subsection{Structural load: how much the offense's wiring changes when a player is removed}
\label{sec:load}
...
renormalizing. \emph{Structural load} is the cosine displacement between the two fingerprints:
\begin{equation}
\label{eq:load}
...
\caption{\textbf{What structural load measures: leave-one-out displacement of the play-type
...
profile $\pt_p$, shown only for context. \emph{Left:} Rudy Gobert (\textsc{Connector/Hub},
...
structural load visible as the \emph{pattern} (not merely the magnitude) of play-type
...
\subsection{Usage-residual structural load}
\label{sec:resid}

To guard against structural load being a relabeling of usage, we regress it on a quadratic in
...
\end{equation}
Residual load $\resid(p)$ is that same construct after removing the component predicted by
usage: structural load that exceeds the level predicted by usage alone. Equivalently,
$\resid(p)$ is the organizational footprint that survives after accounting for involvement; we
show it is stable within players and orthogonal to value. Here $u$ is the \emph{graph-native}
...
distinct, standard NBA measure. We residualize on the graph quantity and report correlations
against the box-score one, so the weak residual--usage correlations in Table~\ref{tab:ortho}
are not mechanical.

\subsection{The role taxonomy: making connectors visible}
\label{sec:taxonomy}

Crossing usage (consumption) with \emph{usage-residual} structural load at a principled
...
$\resid(p)=0$: whether the player carries more structural load than usage alone predicts
(Eq.~\ref{eq:resid}). Unequal class sizes are expected, most players sit at or below
expected load, and are not a defect of the taxonomy.
\begin{equation}
\label{eq:taxonomy}
\text{role}(p)=
\begin{cases}
\textsc{Primary Organizer} & u_p\ge u_{\text{med}} \ \wedge\ \resid(p)> 0,\\[2pt]
\textsc{Connector / Hub} & u_p< u_{\text{med}} \ \wedge\ \resid(p)> 0,\\[2pt]
...
\end{cases}
\end{equation}
Raw-load median splits would force roughly equal quadrants and mislabel high-usage players
whose load is only mechanically elevated by volume (negative residual). The residual cut
...
high-volume creators/finishers whose load is only what usage already predicts
...
illustration of that construct (\textsc{Connector/Hub}), not a four-class research
...
%   every rotation player-season with x = usage, y = structural load, quadrants
...
Structural load is a scalar; portability requires the \emph{direction} of a player's footprint.
...
$\load(p)$ & structural load = how much the offense's wiring depends on $p$, Eq.~\eqref{eq:load} \\
$\resid(p)$ & usage-residual structural load, Eq.~\eqref{eq:resid} \\
...
The portability and efficiency analyses are \emph{pre-registered}. For each we fixed, before
...
what lets us read the efficiency null as an informative boundary rather than a failure to
re-run.
...
(Sec.~\ref{sec:portability}); can we predict transfer (Sec.~\ref{sec:travelfit}).
...
cannot: \textsc{Connector/Hub} players have structural load ($0.078$) comparable to
...
load ($+0.062$) is the highest of any class. \textsc{Terminal Scorers} are the mirror
image, high usage ($0.233$), real scoring ($16.6$ ppg), near-zero residual load ($-0.019$).
Most of the pool sits in the two non-excess-load classes (terminal $40\%$, specialist $39\%$):
...
displacement of the wiring at roughly half the scoring volume. We omit raw on-court net rating
...
initiation share $u$ in Eq.~\ref{eq:resid}). Load is $\load(p)$ (Eq.~\ref{eq:load}); residual
load is $\resid(p)$ (Eq.~\ref{eq:resid}). Classes use usage median $\times$ $\resid>0$
(Eq.~\ref{eq:taxonomy}), not equal-mass raw-load medians. Raw on-court net rating is omitted
from this table and examined as a falsification test in Sec.~\ref{sec:orthogonality}. The
\textsc{Connector / Hub} row is the most visible illustration of the construct; other rows are
contrast.}
\label{tab:taxonomy}
\begin{tabular}{lrrrrrrr}
\toprule
Class & $n$ & \% pool & Usage & Load & Resid.\ load & PPG & MPG \\
\midrule
\textsc{Primary Organizer} & 96 & 10.4 & 0.244 & 0.094 & $+0.054$ & 18.4 & 29.6 \\
\textsc{Connector / Hub} & 103 & 11.1 & 0.145 & 0.078 & $+0.062$ & 9.0 & 23.6 \\
...
Our graph does not track assists, within-possession passing chains, screens set without
initiation credit, gravity, or communication (Sec.~\ref{sec:data}). An \emph{initiator} is
the player who begins the labeled action: ball-handler on a pick-and-roll, cutter on a cut,
post player on a post-up, shooter on a spot-up. Class-average initiation diets make the
roles concrete.
...
\textsc{Connector/Hub} players are not primary handlers. Their diet is cut-and-roll
...
offense's geometry depends on. That is what usage cannot see, and why connectors are the
...
rate and load is $\load(p)$, as in Table~\ref{tab:taxonomy}; residual load is usage-adjusted
$\resid(p)$. Bold-season rows are the points labeled in Fig.~\ref{fig:taxonomy-map} (the
Organizer/Terminal/Connector story in the text). Remaining rows are additional in-class
examples. All organizers and connectors have $\resid>0$; terminals and specialists have
...
\toprule
Class & Player & Team & Season & Usage & Load & Resid.\ load \\
\midrule
\textsc{Organizer}
...
If structural load merely relabeled usage, scoring, or net rating, the measurement would be
uninteresting. We treat on-court net rating as a falsification test of that hidden-premium
hypothesis (Sec.~\ref{sec:intro}): either an apparent connector edge that team quality
explains away, or no apparent edge at all, would falsify a hidden connector value premium.

Under the residual cut, organizers already lead connectors on raw on-court net rating
($+1.45$ vs.\ $+0.80$), so there is no apparent ``connector premium'' to begin with. Once net
rating is residualized within team-season, the organizer median remains above the connector
...
(Table~\ref{tab:ortho}) confirm the separation: usage-residual load is essentially
\emph{uncorrelated with net rating}, $\rho(\resid,\text{net rating})=-0.018$. Raw load
...
(Sec.~\ref{sec:orthogonality}), we check whether the class corresponds to recognizable
offensive behavior: the class-level version of the rim-running, cut-and-roll interior role
illustrated by connector exemplars in Table~\ref{tab:exemplars}, not a biography of any one
player. Using within-team-season comparisons on the on-court subset, we ask whether connector
presence is associated with changes in play-type mix. This corroborates that the structural
class is visible on the floor.

\paragraph{Connector presence and play mix.}
Across $73$ team-seasons with both strata, connector presence shifts the mix toward off-ball
...
interpretation especially clean: a mix shift when connectors are on the floor is less
mechanically attributable to the connector simply taking more of his own shots, and more
...
depends on the class, since organizer and connector presence are not perfectly co-located across
rosters. The observed play-type mix shifts with connector presence.

Two caveats bound these as \emph{associations}, not causal estimates: high-usage classes
mechanically run more of their own play types when present (which is why the connector
...
Connector presence appears to re-channel an offense's existing repertoire rather than expand
...
play-type route, we find no increase in route entropy or reduction in concentration for other
roles under connector presence. The intuitive alternative, that more connectors create
\emph{more} distinct pathways, is not supported. Connector presence is associated with
\emph{which} actions the offense runs within a repertoire bounded by the personnel on the
floor, not with a wider menu.

% RECOMMENDED FIGURE (Figure 6): (a) diverging bar chart of connector within-team
%   play-mix deltas; (b) route-entropy connector-present vs absent (near-zero delta),
...
offensive points per possession. We pre-register the tempting claim that a structurally
...
n^{\text{conn}}_i\ge 1 \ \wedge\ n^{\text{term}}_i\ge 1\,\big],
\end{equation}
and estimate
\begin{equation}
\label{eq:coverage}
...
completeness does not improve efficiency beyond talent at any level we can detect.} Precision
...
effect at $80\%$ power of $\approx2.2$ points per $100$. We can therefore rule out a
...
effect), and the estimate is small and stable across specifications, we read this as a
...
connectors by construction, but the talent-removed specification gives a materially identical
...
\paragraph{Isolating connector presence alone.}
\textsc{Covered} tests whether having the three structural functions on the floor
(organizer, connector, and terminal; Eq.~\ref{eq:covered}) buys points. The fourth taxonomy
class, \textsc{Role Occupant / Specialist}, is not required for completeness: low usage and
low load means it does not fill a creation, connection, or conversion job. A narrower, more
direct version of the practitioner's question is whether merely having a
connector present, regardless of completeness, changes scoring. We reran the identical
team-season-demeaned design with $\textsc{Covered}$ replaced by
$\text{HasConn}_i=\mathbb{1}[n^{\text{conn}}_i\ge1]$, retaining the linear role counts so the
coefficient is connector presence net of accumulation. On the same $N=4.2\times10^{5}$
possessions, the raw within-team-season gap (connector on floor minus off, no other controls)
is $+0.25$ points per $100$; with the full control set,
$\widehat{\text{HasConn}}=+0.00$ points per $100$ ($t=0.00$, $p=1.00$), team-season
cluster-bootstrap $95\%$ CI $[-1.37,+1.48]$. The linear $n_{\text{connector}}$ accumulation
term is nominally different from zero at conventional significance ($+1.17$ per $100$,
$t=2.25$, $p\approx0.02$) but does not clear the pre-registered $p<0.01$ confirmatory bar, so we
do not treat it as a confirmed accumulation effect; it is smaller and less robust than organizer
accumulation, which is robust in every specification in this section, and we do not re-estimate
it here and refer to Table~\ref{tab:coverage}'s single frozen figure ($+2.35$, $t=4.62$).
Whether we ask about completeness or about connector presence
alone, the answer is the same null: connectors reorganize what an offense runs
(Sec.~\ref{sec:behavior}) without moving offensive points per possession.
These tests are \emph{offensive} PPP (points scored while the connector is on the floor for
...
$90$ team-seasons with team-season fixed effects and an offense-only design, connector
presence, whether measured as completeness or in isolation, is not distinguishable from a
null effect on offensive PPP.

\subsubsection{Exploratory: does the HasConn null hide tier heterogeneity?}
\label{sec:connector-tiers}
As a post-hoc check (motivated by an out-of-sample 2025--26 review; Sec.~\ref{sec:data}),
we split \textsc{Connector/Hub} at within-class median residual load into hub vs.\
...
Table~\ref{tab:connector-tiers}: \emph{none} of the three tiers clears the paper's $0.01$
confirmatory bar. The check is exploratory relative to the pre-registered efficiency
tests and does not revise Table~\ref{tab:coverage} or the \textsc{HasConn} null above.

\begin{table}[h!]
\centering\small
\caption{Exploratory connector-tier PPP effects ($N\!=\!420{,}993$, $90$ team-seasons).
Coefficients in points per $100$; confirmatory bar $p<0.01$ as reporting benchmark only.}
\label{tab:connector-tiers}
\begin{tabular}{lrrl}
\toprule
...
the licensed ``so what'' must be organizational. We therefore test identity displacement,
not a second scoring screen, and we do not treat the result as part of the original
confirmatory protocol.

\paragraph{Not the same quantity as $\load(p)$.}
Structural load (Eq.~\ref{eq:load}) is a \emph{season-level leave-one-out counterfactual}:
...
\textsc{Connector/Hub} versus \textsc{Role Occupant / Specialist} is built from
...
and $\ge 400$ off), we split the team's actual possessions by whether $p$ was
...
\paragraph{Design.}
Among $908$ rotation player-seasons meeting the on/off sample gate, we compare class means
of $D_A$, then (as the licensed contrast) match each \textsc{Connector/Hub} to a
...
Terminal scorers are a secondary matched contrast (fewer pairs within caliper). We also
report $\operatorname{corr}(\load,D_A)$ after residualizing both sides on $\log\min(n_{\mathrm{on}},n_{\mathrm{off}})$
and usage, as a continuous construct check.

\paragraph{Result.}
Unmatched means already separate the taxonomy along identity, not only along the
leave-one-out load used to build it (Table~\ref{tab:identity-resilience}): organizers and
connectors show roughly twice the on/off fingerprint displacement of terminals and role
occupants. The primary matched test is decisive: among $96$ connector--role-occupant pairs
(mean usage gap $0.008$; $81$ within the same team-season), mean paired
$\Delta D_A=+0.115$ ($t=6.35$, $p\approx 7\times 10^{-9}$), with the connector larger in
$81\%$ of pairs. Play-type mix also shifts more under connector absence
(paired $\Delta D_\pi=+0.011$, $t=4.06$, $p\approx 10^{-4}$). The thinner
connector--terminal match ($n=25$) agrees in sign for $D_A$
...
connectors alone ($r=+0.79$), ecological validity for the season-level construct, not a
...
connector vs.\ role-occupant contrast in the text ($\Delta D_A=+0.115$, $n=96$ pairs).}
...
Efficiency does not move with connector presence (Sec.~\ref{sec:efficiency}); offensive
\emph{identity} does, under real substitution, relative to usage-matched non-organizers.
The construct-native value of the connector class, on this evidence, is contribution to
maintaining the team's measured association fingerprint when that player is on the floor, not
a PPP or dollar premium. Because the test is post-hoc, we report it as a disclosed
...
(Secs.~\ref{sec:behavior}--\ref{sec:efficiency}) and bounded for efficiency, we turn to that
transfer question. If a player's high structural load reflects something about the player
rather than only the old team around him, his measured wiring footprint should
reappear after he changes teams. We answer with a pre-registered portability test on all
movers, then examine the connector subclass as a measurement stress test: the class where
...
plays everywhere''). We neutralize it with \emph{profile-matched decoys}: for each mover we take
...
exclude origin-side selection; we make no causal claim.
...
$\Delta=+0.134$ ($70.0\%$ portable), connector/hub $+0.124$ ($58.3\%$), terminal scorer
...
(Kruskal--Wallis $H=3.85$, $p=0.28$), so at $n=24$ connector movers we cannot say organizer
and connector medians are statistically distinguishable from each other, from terminal, or in
most pairwise cases from role occupant either -- only that role occupant's much lower
\%-portable is the most visually salient gap in an otherwise non-significant omnibus test. We
report these four numbers as descriptive summary statistics of this sample, not as a confirmed
class ranking, and we do not claim organizers travel better than connectors, connectors
travel better than terminals, or any other pairwise ordering among the four classes. The
connector stress test is therefore about measurement clarity (low usage with load above
usage expectation), not a claim that glue guys are the least (or most) portable class. The
rest of this section and Sec.~\ref{sec:travelfit} stay inside connectors, where the
...
\textsc{Connector / Hub} & 24 & $+0.124$ & $58.3\%$ \\
...
\paragraph{Connector portability is a distribution.}
\label{para:conn-dist}
Figure~\ref{fig:connector-dist} shows realized $\Delta$ among connector movers ($n=24$), with
...
team structure differ. For each mover we therefore compare the direction and relative
...
% Connector-mover Δ distribution (Gafford / Jones Jr. / Wright marked).
\begin{figure}[H]
  \centering
 \includegraphics[width=0.92\textwidth]{fig_connector_delta_dist.png}
 \caption{\textbf{Portability is a distribution, not an invariant.} Realized portability
 $\Delta$ among \textsc{Connector/Hub} movers ($n=24$). Positive $\Delta$ means the player's
...
\begin{table}[t]
\centering\small
\caption{Connector distribution endpoints and median contrast (confirmatory sample;
illustrations, not independent evidence). All three rows are \textsc{Connector/Hub}. Init.\
share $u$ is the graph-native initiation share (Eq.~\ref{eq:resid}). Load and $u$ are
...
\toprule
Case & Player & Init.\ $u$ & Load (o$\to$d) & self-sim & $\Delta$ & Portable \\
\midrule
Strong connector & D.\ Gafford & 0.036 & $0.335\to0.007$ & $+0.86$ & $+0.60$ & \textbf{Yes} \\
Near connector median & D.\ Jones Jr. & 0.030 & $0.025\to0.014$ & $+0.21$ & $+0.17$ & \textbf{Yes} \\
Weak connector & D.\ Wright & 0.016 & $0.030\to0.002$ & $-0.66$ & $-0.71$ & \textbf{No} \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Does destination context explain the variation?}
Having established heterogeneity (Fig.~\ref{fig:connector-dist}), the natural front-office
hypothesis is that \emph{fit} decides: wiring ability reappears if the new roster has a hole
for it, and fails if an incumbent already occupies that organizational slot. We test that with
...
\emph{Result.} Among connector movers ($n=24$), fit does not predict signature excess:
...
\subsection{Can we predict when organizational function transfers?}
\label{sec:travelfit}
...
connectors. Named endpoints belong to Fig.~\ref{fig:connector-dist}; we do not re-litigate them.

\paragraph{What we evaluated.}
...
\begin{itemize}[leftmargin=1.6em,itemsep=0.15em]
 ...
 \item composite travel score (leave-one-out load + stability + usage);
 \item origin structural load, usage, and year-to-year signature stability;
 \item play style (cut vs.\ drive origin diet);
 \item position within connectors; origin$\to$destination role transition;
...
$n=294$), and null within connectors as a continuous predictor of $\Delta$ ($r\approx 0$).
...
shows a consistent directional pattern within connectors, though, as detailed below, it does
...
Origin cut share is the most consistent pre-move signal that separates connector
reproduction among the candidates examined here, but it does not clear conventional
significance as a confirmed within-class screen. On the pre-registered connector-mover
...
is similarly marginal ($\rho=+0.38$, $p=0.07$). Descriptively, high-cut connectors are
$75\%$ portable (median $\Delta=+0.196$); low-cut connectors are $37.5\%$ portable (median
$\Delta=-0.061$). The effect is still more connector-specific than not: within-class
$\operatorname{corr}(\text{cut},\Delta)$ is $+0.32$ for connectors vs.\ $+0.12$
(organizers), $+0.06$ (terminals), $+0.10$ (role occupants), though the connector estimate
itself is not individually significant. Partial correlations controlling for signature
concentration/norm, initiation share, and origin load/usage jointly are consistent in sign
...
temporal halves clears a minimum sample size. We read cut-share as a directionally
...
within-connector rule do not pass.
...
Under our operationalizations, destination vacancy and ``buy the high-load glue guy'' are the
wrong places to start. Origin cut vs.\ drive diet is the leading directional candidate within
connectors, but it is exploratory relative to the pre-registered portability test, modest
($\sim\!10\%$ of within-connector rank variation in $\Delta$), and not statistically confirmed
...
measurable. That changes \emph{what questions we can ask}, not merely which answer we got
to the first efficiency question we posed. Contribution~A is a scientific instrument:
...
player-associated
rather than entirely determined by team context. Connectors make that
...
drive is an exploratory, non-confirmed within-connector candidate
...
connector absence displaces the observed association fingerprint more than
usage-matched role-occupant absence. Efficiency does not move; identity does. That is the
scoped ``so what'' for the connector class in this paper: organizational continuity under
...
load-bearing but low-usage player (a connector) does not measurably harm offensive
...
valuable in points is not organizational centrality, connectors are, by construction,
the class this instrument identifies as load-bearing, but replaceability: NBA rosters and coaching
staffs appear well-equipped to reallocate responsibility among connector-type personnel
without cost, while high-usage organizational talent is not similarly substitutable.
Structural load identifies who an offense's current wiring depends on; it does not identify
who the offense cannot afford to lose.

\paragraph{What becomes measurable now: from players to teams.}
The framework is not only about individuals. Structural load is a team-level distribution,
...
load concentration to a confirmatory team-brittleness claim is still future work.
...
\end{itemize}
These are decision-relevant questions that do not require showing that connectors are worth
...
instrument for decision support without any claim that structural load is itself a value
...
on-court lineups, which opens a symmetric question we do not answer: which defenders reshape
the \emph{opponent's} offensive wiring the most. That requires its own construct-validity work
before it can be evaluated with the same discipline applied to portability. A related, more
immediate extension does not require new defender-attribution work: because connector absence
is already associated with a specific, measured shift in offensive play selection
(Sec.~\ref{sec:behavior}), a defense could in principle use that known pattern to anticipate
not just which player to pressure, but which actions the offense is likely to pivot toward
once that player is removed.
...
Given a roster, structural load ranks which player's removal most displaces the measured
...
predictor under the measures tested here (Sec.~\ref{sec:portability}). Within connectors,
...
Load concentration across a roster is a natural candidate for distinguishing
...
Structural load quantifies how much of an offense's identity rests on a player. The
disclosed on/off identity test (Sec.~\ref{sec:identity-resilience}) finds that connector
absence displaces the measured association fingerprint more than absence of a
usage-matched role occupant, even though connector presence does not move PPP
(Sec.~\ref{sec:efficiency}). A practical question before an absence: \emph{whose removal
most rotates our fingerprint, and how does the wiring reorganize?}

\subsection{Scouting and research: find the wiring first}
Structural load surfaces players who carry organizer-level load without finishing volume.
...
\begin{itemize}[leftmargin=1.4em,itemsep=0.2em]
 ...
 initiation alone: a strength for reproducibility, a limit on mechanism.
 ...
 \item \textbf{Small-load estimation noise.} Near $\resid=0$, taxonomy boundaries are
 ...
 at $n=24$ connector movers; not part of the pre-registered portability protocol
 ...
 \item \textbf{Large residual outside the initiation graph.} Most within-connector $\Delta$
...
jointly hold multiple jobs. Structural load quantifies each player's contribution to that
...
connector-type players make that component visible where volume metrics hide it. A disclosed
post-hoc on/off test then finds the construct-native follow-up to the efficiency boundary:
connector absence displaces offensive identity more than usage-matched role-occupant
...
\paragraph{B. Structural-load robustness.} Frobenius-distance load as an alternative to cosine
(Eq.~\ref{eq:load}); within-player load coefficient of variation across seasons as a stability
check.
...
\paragraph{G. Connector roster (2025--26).} Table~\ref{tab:connector-appendix} lists every
\textsc{Connector/Hub} player-season in the 2025--26 rotation pool
(Sec.~\ref{sec:emerge}), with the same usage, load, MPG, and PPG fields reported in
...
% <<<CONNECTOR_APPENDIX_BEGIN>>>
\begin{longtable}{llrrrr}
\caption{\textsc{Connector/Hub} players in 2025--26 ($n=41$). Usage is box-score usage rate; load is $\load(p)$ (Eq.~\ref{eq:load}); MPG and PPG are box-score minutes and points per game. Sorted by team, then load (descending).}
\label{tab:connector-appendix}\\
\toprule
Player & Team & Usage & Load & MPG & PPG \\
\midrule
\endfirsthead
