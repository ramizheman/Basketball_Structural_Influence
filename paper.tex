...
This paper treats that dependence as a network problem. Represent an offense as a graph linking
...
A null on scoring does not make the measurement useless: knowing whose absence would
disrupt a team's structure most is valuable on its own, separate from any claim about
player value.

With this framework, researchers can ask which offenses would reorganize most if a key
...
the $K=8$ initiating play types and enters every association-matrix and structural-load
...
To guard against structural load being a relabeling of usage, this paper regresses it on a quadratic in
...
player. Using within-team-season comparisons on the on-court subset, this paper asks whether connector
...
connector present, regardless of completeness, changes scoring. This paper reran the identical
...
Whether the question is completeness or connector presence
...
this paper splits \textsc{Connector/Hub} at within-class median residual load into hub vs.\
...
(Kruskal--Wallis $H=3.85$, $p=0.28$), so at $n=24$ connector movers one cannot say organizer
...
class ranking, and does not claim organizers travel better than connectors, connectors
...
connectors. Named endpoints belong to Fig.~\ref{fig:connector-dist}; this paper does not re-litigate them.
...
Under these operationalizations, destination vacancy and ``buy the high-load glue guy'' are the
